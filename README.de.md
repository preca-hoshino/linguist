...
| --- | --- |
| **Nahtlose Multi-Protokoll-Integration** | Native Unterstützung von OpenAI-, Anthropic- und Gemini-Formaten mit bidirektionaler automatischer Konvertierung. |
| **Hochverfügbares Routing** | Feingranulares Modell-Mapping, intelligenter Lastausgleich und automatisches Failover stellen die Dienstkontinuität sicher. |
| **Zero-Downtime Hot-Reload** | Angetrieben durch PostgreSQL `LISTEN/NOTIFY`, werden Konfigurationsänderungen in Millisekunden im Cluster synchronisiert. |
| **Vollständiger Lebenszyklus-Beobachtbarkeit** | Zeichnet Gateway-Anfragen präzise auf und bietet tiefgreifendes Tracing für das Debuggen von Fehlern und Token-Kosten. |
| **Moderne Admin-Konsole** | Out-of-the-Box Admin-API gepaart mit einem visuellen Dashboard, um alle Anbieter-Ressourcen zentral zu verwalten. |
...
- Chat-Endpunkt (OpenAI-Format): `POST http://localhost:3000/model/openai-compat/v1/chat/completions`
...
| Format | Beschreibung    | Chat-Endpunkt                           | Embedding-Endpunkt                      |
| ------ | --------------- | --------------------------------------- | --------------------------------------- |
| OpenAI | Kompatibles Format | `/model/openai-compat/v1/chat/completions`                 | `/v1/embeddings`                       |
| Gemini | Natives Format  | `/v1beta/models/:model:generateContent` | `/v1beta/models/:model:embedContent`   |
...
| `GET`   | `/v1/models`                              | Modellliste abrufen             |
| `POST`  | `/model/openai-compat/v1/chat/completions`                    | OpenAI Chat-Vervollständigung   |
| `POST`  | `/v1/embeddings`                          | OpenAI Text-Embedding           |
| `POST`  | `/v1beta/models/:model:generateContent`   | Gemini Chat-Vervollständigung   |
| `POST`  | `/v1beta/models/:model:streamGenerateContent` | Gemini Streaming-Chat         |
| `POST`  | `/v1beta/models/:model:embedContent`      | Gemini Text-Embedding           |

### Erweiterte Parameter

Weitere Parameter, die von bestimmten Modellen unterstützt werden, finden Sie in der offiziellen Anbieterdokumentation.
...
| Pfad                  | Beschreibung                                      |
| --------------------- | ------------------------------------------------- |
| `/api/providers`      | Anbieter verwalten (CRUD)                         |
| `/api/provider-models` | Anbietermodelle verwalten (CRUD)                  |
| `/api/virtual-models` | Virtuelle Modelle verwalten (CRUD)                |
| `/api/request-logs`   | Protokolle abfragen (unterstützt Filterung)       |
| `/api/api-keys`       | Benutzer-API-Schlüssel verwalten (CRUD & Rotation) |
