...
| --- | --- |
| **Intégration multi-protocoles transparente** | Prend en charge nativement les formats OpenAI, Anthropic et Gemini avec une conversion automatique bidirectionnelle. |
| **Routage à haute disponibilité** | Cartographie fine des modèles, équilibrage de charge intelligent et basculement automatique pour assurer la continuité du service. |
| **Rechargement à chaud sans interruption** | Alimenté par `LISTEN/NOTIFY` de PostgreSQL, les modifications de configuration se synchronisent sur le cluster en quelques millisecondes. |
| **Observabilité du cycle de vie complet** | Enregistre avec précision le cycle de vie des requêtes de la passerelle avec un traçage approfondi pour le débogage des erreurs et les coûts des jetons. |
| **Console d'administration moderne** | API d'administration prête à l'emploi associée à un tableau de bord visuel pour gérer de manière centralisée tous les fournisseurs. |
...
| `POST` | `/v1beta/models/:model:embedContent`      | Intégration de texte Gemini      |

### Paramètres étendus

Veuillez vous référer à la documentation officielle du fournisseur pour les paramètres supplémentaires supportés par les modèles spécifiques.
...
| Chemin                | Description                                      |
| --------------------- | ------------------------------------------------ |
| `/api/providers`      | Gérer les fournisseurs (CRUD)                    |
| `/api/provider-models` | Gérer les modèles de fournisseur (CRUD)          |
| `/api/virtual-models` | Gérer les modèles virtuels (CRUD)                |
| `/api/request-logs`   | Interroger les journaux (supports du filtrage)   |
| `/api/api-keys`       | Gérer les clés API utilisateur (CRUD & rotation) |
