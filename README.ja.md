...
| 機能 | 説明 |
| --- | --- |
| **複数プロトコルのシームレス統合** | OpenAI、Anthropic、Gemini 形式をネイティブにサポートし、双方向の自動変換を行います。 |
| **高可用性ルーティング** | 詳細なモデルマッピング、スマートな負荷分散、自動フェイルバックにより、サービスの継続性を確保します。 |
| **ゼロダウンタイムのホットリロード** | PostgreSQLの `LISTEN/NOTIFY` により、設定変更はミリ秒単位でクラスター全体に同期されます。 |
| **完全なライフサイクル可観測性** | エラーのデバッグとトークンコストの追跡用に、ゲートウェイリクエストの全ライフサイクルを正確に記録します。 |
| **最新の管理コンソール** | 標準装備の Admin API と視覚的なダッシュボードを組み合わせ、すべてのプロバイダリソースを一元管理します。 |
