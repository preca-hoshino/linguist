...
| PUT/PATCH | 路径                       | 说明                                                                                        |
| --------- | -------------------------- | ------------------------------------------------------------------------------------------- |
| GET       | `/api/providers`           | 列出所有提供商（含凭证和高级配置），返回 `{ object: "list", data: Provider[] }`             |
| POST      | `/api/providers`           | 创建提供商（支持 credential_type / credential / config 字段）                               |
| GET       | `/api/providers/:id`       | 获取单个提供商详情                                                                          |
| PATCH     | `/api/providers/:id`       | 更新提供商（部分字段更新，凭证留空则保持原值）                                              |
| DELETE    | `/api/providers/:id`       | 删除提供商                                                                                  |
| GET       | `/api/provider-models`     | 列出提供商模型（可按 provider_id 过滤），返回 `{ object: "list", data: ProviderModel[] }`   |
| POST      | `/api/provider-models`     | 创建提供商模型                                                                              |
| GET       | `/api/provider-models/:id` | 获取单个提供商模型                                                                          |
| PATCH     | `/api/provider-models/:id` | 更新提供商模型                                                                              |
| DELETE    | `/api/provider-models/:id` | 删除提供商模型                                                                              |
| GET       | `/api/virtual-models`      | 列出虚拟模型（含后端列表），返回 `{ object: "list", data: VirtualModel[] }`                 |
| POST      | `/api/virtual-models`      | 创建虚拟模型（可同时关联后端）                                                              |
| GET       | `/api/virtual-models/:id`  | 获取单个虚拟模型详情                                                                        |
| PATCH     | `/api/virtual-models/:id`  | 更新虚拟模型（可同时替换后端）                                                              |
| DELETE    | `/api/virtual-models/:id`  | 删除虚拟模型                                                                                |
| GET       | `/api/request-logs`        | 查询请求日志（分页），返回 `{ object: "list", data: RequestLog[], total: number }`          |
| GET       | `/api/request-logs/:id`    | 获取单条请求日志详情（含完整 GatewayContext 审计快照）                                      |
| GET       | `/api/api-keys`            | 列出所有 API Key，返回 `{ object: "list", data: ApiKey[] }`                                 |
| POST      | `/api/api-keys`            | 创建用户 API Key                                                                            |
| GET       | `/api/api-keys/:id`        | 获取单个 API Key 详情                                                                       |
| PATCH     | `/api/api-keys/:id`        | 更新 API Key（名称、过期时间、启用状态等）                                                  |
