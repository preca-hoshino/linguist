...
import { getUserChatAdapter, getUserEmbeddingAdapter } from '../users';
import { dispatchChatProvider, dispatchEmbeddingProvider } from '../providers/caller';
...
import { handleError, buildErrorResponseBody } from '../users/error-formatting';
import { markProcessing, markCompleted, markError } from '../db';
import { getApiKeyExtractor, getFormatLogger } from '../api';
import { finalizeSuccess, finalizeError, sanitizeHeaders, expressHeadersToRecord } from './helpers';
import { processStreamSend } from './stream';

// 中间件列表
const requestMiddlewares: Middleware[] = [apiKeyAuth, normalizeChatToolCallIds];
const responseMiddlewares: Middleware[] = [normalizeResponseChatToolCallIds];
...
async function processRequest(
  ...
  try {
    ...
    if (isStream) {
      ...
      if (isChat) {
        await dispatchChatProvider(ctx, ctx.request as InternalChatRequest);
      } else {
        await dispatchEmbeddingProvider(ctx, ctx.request as InternalEmbeddingRequest);
      }
    ...
    finalizeError(ctx, err, label, logger);
    if (res.headersSent) {
      res.end(); // 流式传输中途出错，只能关闭连接
    } else {
      handleError(err, res, userFormat);
    }
    // 记录错误响应审计数据（与 handleError 构建完全一致的响应体）
    const errorPayload = buildErrorResponseBody(err, userFormat);
    ctx.audit.userResponse = {
      headers: expressHeadersToRecord(res.getHeaders()),
      body: errorPayload.body,
    };
    void markError(ctx, err);
  }
}
