...
import type { GeminiTool, GeminiToolConfig, GeminiFunctionDeclaration } from './types';
...
function cleanSchemaForGemini(schema: Record<string, unknown>): Record<string, unknown> {
  ...
  if (normalizedType === null || normalizedType === 'null') {
    ...
    normalizedType = filtered.length === 1 ? filtered[0] : filtered.length > 0 ? filtered : undefined;
  }
