...
import { extractErrorObj, extractString, fallbackByStatus, tryParseJson } from '@/providers/errors';
...
export function mapGeminiError(httpStatus: number, body: string): ProviderErrorInfo {
  ...
  const providerErrorCode = errorObj === null ? undefined : extractString(errorObj, 'status');
