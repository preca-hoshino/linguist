// src/model/http/providers/newapi/chat/request/message-converter.ts — New API 消息处理
//
// New API 使用 OpenAI 兼容的消息格式，与 DeepSeek 完全一致
...
/**
 * 将单个内部 ContentPart 转换为 OpenAI API 格式
 *
 * New API 使用 OpenAI 兼容的多模态格式：
 * - TextContentPart → { type: 'text', text }
 * - MediaContentPart (image) → { type: 'image_url', image_url: { url } }
 * - 其他媒体类型（audio/video/file）New API 不支持，跳过并记录警告
 */
function convertContentPart(part: ContentPart): Record<string, unknown> | null {
  ...
  // audio/video/file：New API 当前不支持
  logger.warn({ type: part.type }, 'New API does not support this media type; skipping');
  return null;
}

/**
 * 将内部消息的 content 字段转换为 OpenAI API 格式
 *
 * - string → 保持原样
 ...
 * - assistant 消息若携带 reasoning_content，则原样传递给 New API（满足多轮对话需求）
