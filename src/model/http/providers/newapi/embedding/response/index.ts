// src/model/http/providers/newapi/embedding/response/index.ts — New API 嵌入响应适配器
//
 ...
 * 将兼容 OpenAI 格式的 /v1/embeddings 响应转换为内部通用格式
 */
export class NewApiEmbeddingResponseAdapter implements ProviderEmbeddingResponseAdapter {
  public fromProviderResponse(providerRes: unknown): InternalEmbeddingResponse {
    logger.debug('Adapting New API embedding response to internal format');

    if (providerRes === undefined || providerRes === null || typeof providerRes !== 'object') {
      throw new Error('Invalid New API embedding response: not an object');
    }
