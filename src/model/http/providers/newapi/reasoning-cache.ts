// src/model/http/providers/newapi/reasoning-cache.ts — New API 推理内容缓存
//
// New API 代理推理模型（如 DeepSeek R1）时，多轮对话要求 assistant 消息的
// reasoning_content 字段必须在历史消息中原样回传。客户端可能不保留该字段，
// 因此网关在响应后缓存 reasoning_content，在下次请求转发前自动注入。
//
// 与 DeepSeek 提供商共享相同的缓存策略和容量限制。
// 两者使用独立的缓存实例，避免跨提供商的语义混淆。
