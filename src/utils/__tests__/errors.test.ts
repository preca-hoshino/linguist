...
describe('handleError', () => {
  ...
  it('should handle GatewayError with correct status and structure', () => {
    ...
    handleError(err, res as unknown as import('express').Response);
  ...
  it('should handle GatewayError 400 for stream not supported', () => {
    ...
    expect((res.body as { error: { code: string } }).error.code).toBe('stream_not_supported');
  });
  ...
  it('should handle non-Error thrown values', () => {
    ...
    handleError('string error', res as unknown as import('express').Response);

    expect(res.statusCode).toBe(500);
    expect((res.body as { error: { message: string } }).error.message).toBe('string error');
  });
});
