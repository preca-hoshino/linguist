...
**极致性能优化：大跨度时序的预聚合路由 (OLAP Architecture)**

对于跨度大于 24 小时的查询（如按天、按月统计），系统实现了按业务域解耦的物化视图架构，将 `O(N)` 的聚合成本压缩至 `O(1)` 级别：
1. **统一视图规范**：遵循 `mv_stat_[业务域]_[时间粒度]`，目前已建成 `mv_stat_llm_hourly`（未来将扩展 `mv_stat_mcp_hourly` 等）。
2. **极速路由拦截**：当查询 `range` 大于等于 24 小时，`overview.ts` 等入口自动拦截重写 SQL，改走物化视图查阅（响应降至 50 毫秒以内）。
3. **后台免感刷新**：由应用层全局定时器 (`refresh.ts`) 每隔 5 分钟进行无锁 `CONCURRENTLY` 刷新，保证高可用。
