  ...
  const sql = `
    ...
    agg AS (
      SELECT
        ...
        COALESCE(SUM((d.gateway_context->'response'->'usage'->>'prompt_tokens')::bigint), 0)::bigint                             AS sum_prompt_tokens,
        COALESCE(SUM((d.gateway_context->'response'->'usage'->>'completion_tokens')::bigint), 0)::bigint                         AS sum_completion_tokens,
        COALESCE(SUM((d.gateway_context->'response'->'usage'->>'total_tokens')::bigint), 0)::bigint                              AS sum_total_tokens,
        COALESCE(SUM((d.gateway_context->'response'->'usage'->>'cached_tokens')::bigint), 0)::bigint                             AS sum_cached_tokens,
