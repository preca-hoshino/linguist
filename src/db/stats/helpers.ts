...
import type { StatsDimension, StatsRange, StatsInterval, StatsQueryParams } from './types';
...
/**
 * TTFT SQL 表达式（首 Token 延迟 = ttft - start，单位毫秒）
 * @param alias 可选表别名
 */
...
/**
 * ITL SQL 表达式（非首 Token 延迟 = end - ttft，单位毫秒）
 * @param alias 可选表别名（如 'rl'），用于联表查询
 */
export function itlExpr(alias = ''): string {
  const t = alias ? `${alias}.timing` : 'timing';
  return `CASE WHEN ${t}->>'end' IS NOT NULL AND ${t}->>'ttft' IS NOT NULL
    THEN (${t}->>'end')::float - (${t}->>'ttft')::float END`;
}

// 无别名时序用常量（性能优化，避免函数调用）
export const LATENCY_EXPR = latencyExpr();
export const TTFT_EXPR = ttftExpr();
export const ITL_EXPR = itlExpr();
...
export function rangeToInterval(range: StatsRange): string {
