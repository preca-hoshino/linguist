...
async function executeMigrationFiles(executor: QueryExecutor, log: Logger): Promise<void> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      log.error(err instanceof Error ? err : new Error(String(err)), `✗ ${file} failed`);
      throw err;
    }
  }
...
if (require.main === module) {
  ...
  task
    ...
    .then(() => process.exit(0))
    .catch((err: unknown) => {
      logger.error(err instanceof Error ? err : new Error(String(err)), isReset ? 'Reset failed' : 'Migration failed');
      process.exit(1);
    });
}
