...
import { createLogger, buildUpdateSet } from '../../utils';
import { invalidateApiKeyCache, lookupKeyHash } from './cache';
import type { ApiKeySummary, ApiKeyCreateResult } from './types';
...
export async function listApiKeys(): Promise<ApiKeySummary[]> {
  const result = await db.query<ApiKeySummary>(`SELECT ${SUMMARY_COLUMNS} FROM api_keys ORDER BY created_at DESC`);
  return result.rows;
}
